...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    b = min(points, key = lambda x: x[1])
    t = max(points, key = lambda x: x[1])
    for i in points:
        if i != b and i != t:
            m = i
    ...
    if (b[1] == m[1]):
        draw_line(int(x0), int(y0), int(z0), int(m[0]), int(m[1]), int(m[2]), screen, zbuffer, color)

    while (y0 <= t[1]):
        draw_line(int(x0), y0, z0, int(x1), y0, z1, screen, zbuffer, color)
        x0 += dx0
        x1 += dx1
        y0 += 1
        z0 += dz0
